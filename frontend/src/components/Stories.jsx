        ...
        <div className="w-full h-fit flex items-center justify-start gap-4 p-2 overflow-x-auto scrollbar-none overflow-y-hidden scroll-smooth">
            ...
            {stories?.map((story) => (
                <div key={story._id} className="w-fit h-fit flex flex-col items-center justify-center gap-1">
                    <div key={story._id} className={`relative w-16 h-16 rounded-full z-10 ${(story?.views?.some(v => v === userData?.user?._id)) ? 'bg-[#363636]' : 'bg-gradient-to-tr from-yellow-400 via-pink-500 to-purple-600'} flex items-center justify-center`}>
