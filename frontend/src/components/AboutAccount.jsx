...
const AboutAccount = ({ setAboutAcc, theme, country, userPost, page, post, user }) => {
                ...
                < div className={`w-fit h-fit flex flex-col items-center justify-center rounded-2xl  ${theme === 'dark' ? 'bg-[black] border-l-1 border-r-1  border-[#3636367b]' : (theme === 'light') ? 'bg-[#f2f2f2] ' : ' bg-[#f2f2f2] dark:bg-[black]'} `}>
                    ...
                    <div className="md:w-100 h-fit flex flex-col items-center justify-center gap-5 px-3">
                        <div className={`w-full h-fit flex flex-col items-center justify-center gap-2 py-3 `}>
                            <img src={page === 'main' ? post?.author?.profilepic || dp : userPost?.post?.author?.profilepic || user?.profilepic || dp} alt="profile pic" className="w-20 h-20 rounded-full object-cover" />
                            <p className={`text-xs md:text-[13px] font-bold pt-1 ${theme === 'dark' ? 'text-[white]' : (theme === 'light') ? 'text-[#0000007c]' : ' text-black dark:text-[white]'}`}>{page === 'main' ? post?.author?.username || post?.author?.name : userPost?.post?.author?.username || userPost?.post?.author?.name || user?.username || user?.name}</p>
                        ...
                        <div className="w-full h-fit flex flex-col items-start justify-center gap-4">
                            <div className="flex items-center justify-start gap-2">
                                ...
                                <div>
                                    ...
                                    <p className={` text-xs ${theme === 'dark' ? 'text-[#ffffff7c]' : (theme === 'light') ? 'text-[#0000007c]' : ' text-black dark:text-[#ffffff7c]'}`}>{page === 'main' ? post?.author?.createdAt?.slice(0, 10) : userPost?.post?.author?.createdAt?.slice(0, 10) || user?.createdAt?.slice(0, 10)}</p>
                                </div>
                            </div>
                            <div className="flex items-center justify-start gap-2">
