        ...
        <>
            {status === 'loading' ? <div className={`w-full h-100 flex items-center justify-center`}>
                ...
                <div className={`w-full h-fit grid grid-cols-3 gap-1 justify-items-center `}>
                    {posts?.map((post) => (
                        <div key={post._id} onClick={() => { handleClick(post) }} className={`relative w-full ${page && page === 'explore' ? 'h-full min-h-40 md:min-h-70 ' : 'h-40 md:h-90 lg:h-100'} cursor-pointer ${(page && page === 'explore' && post?.type === 'Reel') ? 'row-span-2' : ''  }`}>
