...
const ViewPostImage = ({ handleLike, posts, indexval, liked, page, post, viewReels }) => {
  ...
  return (
    <div className="relative w-full h-full flex flex-col items-center justify-center">
      ...
      {page === 'main' && <div className={`w-full h-full flex flex-col items-center justify-center overflow-hidden`}>
        <div onDoubleClick={() => { handleLike() }} className="relative w-full h-full flex flex-col items-center justify-center">
          <img src={post?.image} alt="post" className={`3xl:w-220 2xl:h-210 w-full h-190 object-cover ${viewReels ? 'h-250' : 'h-90'}`} />
        </div>
      </div>}
      {((posts && posts[indexval]?.tagged)  || (post && post?.tagged) ) && <div onClick={() => { setShowTagged(!showTagged) }} className="absolute bottom-5 left-5">
      ...
      {showTagged && <div className={`absolute bottom-15 left-3 z-60 w-fit h-fit transition-all duration-200 ease-in-out flex items-center justify-center px-3 py-2 rounded-lg ${theme === 'dark' ? 'bg-[#171717]' : (theme === 'light') ? 'bg-white' : ' bg-white dark:bg-[#171717]'}`}>
        ...
        <p className={`text-xs md:text-sm z-10  ${theme === 'dark' ? 'text-white' : (theme === 'light') ? 'text-black' : ' text-black dark:text-white'}`}>{(posts && posts[indexval]?.tagged?.username) || (post && post?.tagged?.username)} </p>
      </div>}
      {liked && (
