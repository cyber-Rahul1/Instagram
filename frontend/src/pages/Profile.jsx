...
const Profile = () => {
  ...
  return (
    <div onClick={() => { setActiveItem('Profile'); setSearchIsFocussed(false); setNotificationIsFocussed(false); }} className={`${theme === 'dark' ? 'bg-black text-white' : (theme === 'light') ? 'bg-white text-black' : ' bg-white dark:bg-black text-black dark:text-white'} flex flex-col items-center justify-start h-screen pt-20 md:pt-0 w-full px-2 md:px-3 lg:px-auto overflow-y-auto transition-all duration-200 ease-in-out`}>
      ...
      <div className="relative w-full lg:w-[950px] h-fit pb-10 flex items-start justify-center pt-5 md:border-b-1 border-[#a8a5a346]">
        ...
        <div className="w-[90%] h-[90%] flex-col md:flex-row flex justify-start md:gap-20 ">
          <div className="flex items-start gap-5 px-auto">
            <div onClick={() => image.current.click()} className="relative w-[95px] h-[95px] cursor-pointer md:w-[145px] md:h-[145px] lg:mx-6 md:my-10 border-1 border-[#a8a5a3a7] rounded-full overflow-hidden flex items-center justify-center">
              <img className="w-full h-full object-cover" src={profileImage || userProfile?.profilepic || dp} alt="dp" />
              {(!userData?.user?.profilepic) && <div className={` ${theme === 'dark' ? 'bg-[#00000071] text-white  hover:bg-[#00000086]' : (theme === 'light') ? 'bg-[#f0f0f0] text-[#b5b4b4] hover:bg-[#cecdcd]' : 'bg-[#f0f0f0] dark:bg-[#686565a4] hover:bg-[#cecdcd] dark:hover:bg-[#68656575]'} absolute w-full h-full rounded-full  flex items-center justify-center`}><FaCamera className="text-2xl md:text-5xl" />
        ...
        <div onClick={(e) => e.stopPropagation()} className="absolute md:hidden bottom-3 flex w-full gap-2 px-2">
